...
def check_candidate_support(pair, data, local_threshold):
...
if __name__ == '__main__':
    ...
    if case_number == 1:
        ...
        businessUser = test_data \
            .map(lambda x: x.split(',')) \
            .map(lambda x: (x[1], [x[0]])) \
            .reduceByKey(combine_list)

        case_data = businessUser \
            .filter(lambda x: x[1][0] != 'user_id') \
            .map(lambda x: (x[0], set(x[1]))) \
            .map(lambda x: (x[0], list(x[1])))
    ...
    apRDD = basket_values.mapPartitions(lambda x: ap(list(x), numBasket,  support)) \
    ...
    #print("num frequent: " + str(len(candidate_itemsets)))
    ...
    sonRDD = basket_values.mapPartitions(lambda x: find_true_freq(list(x), candidate_itemsets)) \
        .reduceByKey(lambda x, y: x + y) \
